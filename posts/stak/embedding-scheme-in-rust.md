...
Rust, as a compiled language, makes it challenging to dynamically modify the behavior of programs. In this article, we embed a small Scheme interprerter written in Rust called [Stak Scheme][stak] is embedded in a Rust program to dynamically change the behavior of a program written in Rust (without stopping the process).
