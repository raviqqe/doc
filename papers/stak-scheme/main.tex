...
to evaluate the compactness.
We compile these executable files for the x86-64 CPU architecture on
the operating system of Ubuntu 24.04 with
the Linux kernel 6.14.1.
For the comparison, we use a default version of Stak Scheme (\texttt{stak})
...
We also use \texttt{mstak} as a baseline, which comes with a number
...
interpreter executable file of Chibi Scheme compiled statically.
