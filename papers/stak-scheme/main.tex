...
Figure \ref{figure:build} shows an example of a workflow for
embedding a Stak Scheme script into a Rust program.
After a user triggers \texttt{cargo build} command, which is of the
Cargo build system for Rust, a \texttt{build.rs} build
...
into a bytecode file \texttt{foo.bc}.
...
\begin{lstlisting}[caption=\texttt{foo.scm} file, label={list:scheme}]
...
\begin{lstlisting}[caption=\texttt{main.rs} file, label={list:rust}]
...
fn run_scheme(module: &UniversalModule) -> Result<(), EngineError> {
    // Initialize a heap memory for an scripting engine.
