...
implementations compliant to the R7RS-small standard in computational
benchmarks shown in Table \ref{table:computation}.
The following list is the benchmarks we run the Scheme interpreters
against.
...
We run each benchmark 10 times with 5 time warmup runs and report their
average time as well as the standard deviation.
...
emulating situations of embedded scripting describe in Section \ref{stak}.
...
Scheme implementations with performance, for example, 1.67 times
faster than \textbf{gsi} in the \textbf{fibonacci} benchmark,
despite its minimal design and implementation.
However, it suffers on the \textbf{sum} benchmark against
\textbf{gosh} where Stak Scheme is not
particularly optimized for but uses tail calls for iterations,
In the benchmarks of \textbf{empty} and \textbf{hello}, Stak Scheme,
especially its bytecode interpreters performs significantly well
compared to the other Scheme implementations.
This does not only proves the lightweightness of the Scheme
implementation of Stak Scheme, but also demonstrates that
the split architecture of a bytecode compiler and a virtual machine
of Stak Scheme as well as Ribbit Scheme is suitable for embedded scripting.
