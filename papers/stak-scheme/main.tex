...
across its modules with checks of symbol visibility and side effects.
...
In addition, Stak Scheme also provides non-standard libraries under
the `stak` namespace including.

\begin{itemize}
  \item The \texttt{(stak base)} library: Arithmetic operations and basic types.
  \item The \texttt{(stak parameter)} library: Parameter objects.
  \item The \texttt{(stak io)} library: I/O operations.
  \item The \texttt{(stak continue)} library: Continuations.
  \item The \texttt{(stak exception)} library: Exception handling.
\end{itemize}
