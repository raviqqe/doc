...
portability and compactness of the bytecode formats on memory
and disk, and the bytecode decoder which the virtual machine is
equipped with while keeping its performance in computation time.
...
As such, the problem of encoding and decoding bytecode falls into
...
beyond its modules boundaries with the analysis of symbol visibility
and side effects.
...
Note that we skew the semantics of Scheme slightly to simplify the
implementation although the tree shaking pass is enabled by default.
...
split out from the standard \texttt{(scheme base)} library under
...
interpreter executable file of Chibi Scheme with its library linked
statically and C compiler flags of \texttt{SEXP_USE_NO_FEATURES=1} and
\texttt{SEXP_USE_FLONUMS=1} to minimize its binary size.
