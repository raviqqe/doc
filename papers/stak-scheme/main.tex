...
The relatively large bytecode size is partially due to lack of
bytecode size optimization from Stak Scheme which Ribbit Scheme
implements already, such as the LZSS compression, RIBN's optimal encoding.
Another reason for the large bytecode size is our full-fledged
\texttt{eval} procedure in the interpreter, which encompasses the
entire bytecode compiler.
Although we can technically implement another interpreter in Scheme
itself which does not integrate the existing Scheme-to-bytecode
compiler like Ribbit Scheme,
