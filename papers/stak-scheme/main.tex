...
\begin{abstract}
  ...
  whereas its performance is still comparable to other
...
been used for such scripting purposes, such as Script-Fu \cite{scriptfu}
...
in-memory format and executes it.
...
itself into the source code, which is described in detail later in
...
Like Ribbit Scheme, we have only two primitive data types on the virtual
machine: numbers and cons cells.
...
virtual machine and its encoding and decoding algorithms in detail.
...
Therefore, the problem of encoding and decoding bytecode reduces to
the problem of encoding and decoding a graph of pairs.
...
\begin{itemize}
  ...
  \item We do not use any complex data structures but only singly-linked lists
    ...
    only primitive data structures of numbers and cons cells available on
    the virtual machine.
\end{itemize}
...
dictionary of \textit{shared} cons cells represented as a list.
...
Across the function, we track how many times the cons cells appear and
decrement their reference counts every time when they show up in the bytecode.
When the reference counts of cons cells reach zero, they are removed from the
...
One of the methods to reduce the size of programs in dynamic programming
...
It not only reduces the size of bytecode but also
...
In addition to typical micro-benchmarks of \textbf{fibonacci},
...
Scheme's computational performance remains comparable to other
Scheme implementations with performance, for instance, 1.65 times
...
particularly optimized for such iteration pattern but uses tail calls for it.
In the benchmarks of \textbf{empty} and \textbf{hello}, our interpreters,
especially their bytecode versions, perform significantly better
than the other Scheme implementations.
This not only highlights the lightweight nature of the Stak Scheme
...
\begin{table*}
  \begin{center}
    \begin{tabular}{l|llllllllllllllllll}
      \hline
      Benchmark & mstak & stak & mstak-interpret & stak-interpret &
      tr7i & gsi & chibi-scheme & gosh \\
      \hline
      empty & 1.00 \pm 0.02 & 1.04 \pm 0.02 & 0.14 \pm 0.00 & 0.38
      \pm 0.02 & 0.77 \pm 0.02 & 0.51 \pm 0.01 & 3.63 \pm 0.02 & 1.27
      \pm 0.04 \\
      fibonacci & 1.00 \pm 0.00 & 1.12 \pm 0.00 & 0.96 \pm 0.00 &
      1.05 \pm 0.00 & 1.35 \pm 0.00 & 1.66 \pm 0.00 & 0.93 \pm 0.00 &
      0.45 \pm 0.00 \\
      hello & 1.00 \pm 0.02 & 1.04 \pm 0.02 & 0.13 \pm 0.00 & 0.36
      \pm 0.01 & 0.73 \pm 0.02 & 0.53 \pm 0.01 & 9.84 \pm 0.03 & 3.62
      \pm 0.04 \\
      sum & 1.00 \pm 0.00 & 1.13 \pm 0.01 & 1.01 \pm 0.00 & 1.06 \pm
      0.00 & 1.19 \pm 0.01 & 1.64 \pm 0.00 & 0.98 \pm 0.00 & 0.24 \pm 0.00 \\
      tak & 1.00 \pm 0.00 & 1.09 \pm 0.01 & 0.89 \pm 0.00 & 0.98 \pm
      0.01 & 0.96 \pm 0.00 & 1.23 \pm 0.00 & 1.21 \pm 0.00 & 0.54 \pm 0.00 \\
      \hline
    \end{tabular}
...
They first coined the basic architecture of Ribbit Scheme
...
That not only makes the implementation of
RVMs simple but makes them portable across different
...
depending on the libraries that source programs import.
...
Such panics are hard to debug, especially when an executable file of
the interpreter is stripped of debug information.
...
Although we believe that our bytecode decoding algorithm works
...
Porting the Stak Scheme virtual machine to other languages
not only proves its portability but also extends its use cases.
...
\begin{acks}
  We are deeply grateful to the dynamic programming language team at the
  ...
  Léonard Oest O'Leary from the team at the University of Montréal provided
  valuable comments including suggestions for improving the
  evaluation sections.
  William E. Byrd gave us general advice on academic writing and provided
